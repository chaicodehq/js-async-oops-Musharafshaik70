...
export class Startup {
  ...
  constructor(name, founder, domain) {
    ...
    this.#funding = 0;
    this.founded = new Date().toISOString();
    const domainsAvailable = ["fintech", "edtech", "healthtech", "foodtech"];
    if (!domainsAvailable.includes(domain))
      throw new Error(
...
export class Incubator {
  ...
  constructor(name, maxStartups) {
    ...
    this.#startups = [];
    this.#mentors = [];
    if (this.maxStartups <= 0) throw new Error("Invaid maxStartups");
    else this.maxStartups = maxStartups;
  }
  ...
  async conductDemo(startupName) {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        const reqStartup = this.#startups.find(
          (s) => s.startup.name === startupName,
        );
        if (!reqStartup) return reject(new Error("Startup not found!"));

        reqStartup.demoCompleted = true;
        const feedbacks = [
          "Bahut badhiya!",
  ...
  async batchProcess(startups) {
    const startupArray = startups.map((startup) => this.admitStartup(startup));
    return await Promise.allSettled(startupArray);
  }
  ...
  getTopFunded(n) {
    ...
    return [...this.#startups]
      .sort((s1, s2) => s2.startup.funding - s1.startup.funding)
      .slice(0, n)
      .map((s) => s.startup);
  }
...
export async function runDemoDay(incubator) {
  ...
  for (const startup of incubator) {
    //since i made incubator iterable using Symbol.iterator
    startups.push(startup);
  }
  const demoTasks = startups.map((startup) =>
    incubator.conductDemo(startup.name),
  );
  const settledResults = await Promise.allSettled(demoTasks);
  return {
    incubator: incubator.name,
